...
def get_distance(feature1, feature2, mask1, mask2, RESOLUTION=64, num_patches=60):
    src_feature_reshaped = feature1.squeeze().permute(1,0).reshape(1,-1,num_patches,num_patches).cuda()
    tgt_feature_reshaped = feature2.squeeze().permute(1,0).reshape(1,-1,num_patches,num_patches).cuda()
...
def get_distance_mutual_nn(feature1, feature2):
    ...
    distances_1to2 = torch.cdist(feature1, feature2)[0]
...
def resize(img, target_res=224, resize=True, to_pil=True, edge=False, padding=True):
    if not padding and resize:
        return img.resize((target_res, target_res), Image.Resampling.BILINEAR)
...
def apply_gaussian_kernel(corr, sigma=5, num_patches=60):
    ...
    x = np.linspace(0,num_patches-1,num_patches)
    x = torch.tensor(x, dtype=torch.float, requires_grad=False).to(corr.device)
    y = np.linspace(0,num_patches-1,num_patches)
...
def get_flow(corr, flow_window=0, num_patches=60):
    ...
    if flow_window > 0:  # zero out the corr_map outside the window
        ...
        corr = apply_gaussian_kernel(corr, sigma=-flow_window, num_patches=num_patches)
...
def calculate_keypoint_transformation(args, img1_desc, img2_desc, img1_patch_idx, num_patches):
    ...
    sim_1_to_2 = torch.matmul(img1_desc, img2_desc.permute(0, 2, 1))[0]
